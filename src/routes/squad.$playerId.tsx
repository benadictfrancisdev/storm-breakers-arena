    ...
    return {
      meta: [
        { title },
        {
          name: "description",
          content: p
            ? `${p.name} (#${p.number}) — ${p.role} for Storm Breakers Cricket Club.`
            : "Storm Breakers player profile.",
        },
...
function PlayerPage() {
    ...
    return (
      <main className="min-h-screen bg-background text-foreground flex items-center justify-center px-5">
        <div className="text-center">
          ...
          <Link to="/" className="mt-6 inline-block text-[var(--electric)] underline">
            Back home
          </Link>
        </div>
      </main>
  ...
  const save = () => {
    update(player.id, {
      ...
      number: number.trim() || player.number,
      role: role.trim() || player.role,
    });
    setEditing(false);
  };
    ...
    <main className="relative bg-background text-foreground overflow-x-hidden min-h-screen">
      ...
      <section className="pt-32 pb-20 px-5">
        <div className="max-w-6xl mx-auto">
          <button
            onClick={() => navigate({ to: "/", hash: "squad" })}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div>
              ...
              {!editing ? (
                <>
                  <h1 className="font-display text-6xl md:text-8xl leading-[0.85] text-gradient-blade">
                    {player.name}
                  </h1>
                  ...
                  <p className="mt-6 text-muted-foreground max-w-md leading-relaxed">
                    Part of the Storm Breakers brotherhood. Edit this profile to add a real photo,
                    change the jersey number, or update the role.
                  </p>
                  <button
